...
export default function InformesClient({ initialInformes, initialPacientes }: { initialInformes: any[]; initialPacientes: any[] }) {
  ...
  const [pacientes, setPacientes] = useState<any[]>(initialPacientes || [])
  ...
  async function handleGenerar() {
    ...
    // invalidate server cache so server pages pick up the new report
    try {
      await fetch('/api/cache/invalidate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ key: 'reports' }),
      })
    } catch (e) {
  ...
  useEffect(() => {
    // marca como cargado después del montaje / inicialización de datos
    setLoading(false)
  }, [])
